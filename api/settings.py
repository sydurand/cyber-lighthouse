...
async def get_all_settings() -> Dict[str, Any]:
    ...
    try:
        ...
        for key, value in settings.items():
            # We need to get category from DB
            pass
        
        # Simpler: just return all settings with metadata
        result = {}
        with __import__('sqlite3').connect(db.db_file) as conn:
            cursor = conn.cursor()
...
async def update_rss_feeds(feeds_update: FeedsUpdate) -> Dict[str, Any]:
    ...
    try:
        feeds_data = [f.dict() for f in feeds_update.feeds]
...
async def add_rss_feed(feed: FeedEntry) -> Dict[str, Any]:
    ...
    try:
        ...
        if any(f.get("name") == feed.name for f in feeds):
        ...
        logger.info(f"RSS feed added: {feed.name}")
        return {"message": f"Feed '{feed.name}' added successfully", "feed": feed.dict()}
    except HTTPException:
        raise
    except Exception as e:
