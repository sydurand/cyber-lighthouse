...
class TestSimilarityDetection:
    ...
    def test_detect_similar_articles_bluehammer_scenario(self):
        """Test clustering articles about same vulnerability with very different titles.
        
        This tests the BlueHammer scenario where three articles cover the same zero-day
        but use completely different wording:
        - Article 1: Mentions "BlueHammer" and "disgruntled researcher"
        - Article 2: Mentions "1 billion Microsoft users" and "angry hacker" (no BlueHammer)
        - Article 3: Mentions "BlueHammer" and "bug disclosure"
        
        All three should be clustered together despite title differences.
        """
        articles = [
            {
                "id": 1,
                "title": "Disgruntled Researcher Leaks BlueHammer Windows Zero-Day Exploit",
                "content": "A security researcher under the alias Chaotic Eclipse has publicly released proof-of-concept code for BlueHammer, an unpatched Windows zero-day flaw allowing local privilege escalation. The vulnerability combines a time-of-check to time-of-use (TOCTOU) and path confusion issue, granting attackers access to the SAM database and enabling escalation to SYSTEM privileges. Microsoft's MSRC has acknowledged the report."
            },
            {
                "id": 2,
                "title": "1 Billion Microsoft Users Warned as Angry Hacker Drops 0-Day Exploit",
                "content": "Over 1 billion Microsoft Windows users are at risk after a frustrated security researcher leaked a zero-day exploit code online. The vulnerability, known as BlueHammer, is a local privilege escalation (LPE) flaw that allows attackers to gain SYSTEM access. The researcher, going by Chaotic Eclipse, expressed frustration with Microsoft's disclosure process. The exploit targets a TOCTOU race condition and path confusion bug in Windows."
            },
            {
                "id": 3,
                "title": "BlueHammer Windows Exploit Highlights Microsoft Bug Disclosure Issues",
                "content": "The recent leak of the BlueHammer Windows zero-day exploit has reignited debate about Microsoft's vulnerability disclosure practices. The exploit, released by researcher Chaotic Eclipse, leverages a time-of-check time-of-use vulnerability to escalate privileges to SYSTEM level. Security experts warn this could be weaponized by threat actors if they gain initial access through social engineering or credential theft."
            }
        ]
        
        groups = detect_similar_articles(articles)
        
        # All three articles should be clustered into the same group
        # (they all discuss the same BlueHammer vulnerability)
        unique_groups = set(groups.values())
        assert len(unique_groups) == 1, f"Expected 1 group, got {len(unique_groups)}: {groups}"
        assert groups[1] == groups[2] == groups[3], f"Articles not grouped together: {groups}"

    def test_detect_similar_articles_bluehammer_partial_match(self):
        """Test that articles with partial overlap still cluster correctly.
        
        Tests edge case where one article mentions the vulnerability but with
        minimal shared vocabulary.
        """
        articles = [
            {
                "id": 1,
                "title": "Critical Windows Zero-Day Allows Privilege Escalation",
                "content": "Microsoft Windows users should update immediately following disclosure of a critical privilege escalation vulnerability. The zero-day flaw allows attackers to gain SYSTEM-level access by exploiting a TOCTOU race condition in the Windows kernel. Security researcher Chaotic Eclipse released proof-of-concept code after becoming frustrated with Microsoft's response timeline."
            },
            {
                "id": 2,
                "title": "BlueHammer: New Windows Exploit Leaked by Disgruntled Researcher",
                "content": "The BlueHammer exploit has been publicly released by a security researcher known as Chaotic Eclipse. This Windows zero-day allows local privilege escalation to SYSTEM privileges through a time-of-check to time-of-use vulnerability combined with path confusion. Microsoft's MSRC team is investigating the issue."
            },
            {
                "id": 3,
                "title": "Unrelated Linux Kernel Update Fixes Security Bugs",
                "content": "The Linux kernel development team has released version 6.8.1 with fixes for several security vulnerabilities. The update addresses buffer overflow issues and improves memory management security. Users are encouraged to update their systems at the earliest convenience."
            }
        ]
        
        groups = detect_similar_articles(articles)
        
        # Articles 1 and 2 should be grouped (both about Windows zero-day/BlueHammer)
        # Article 3 should be separate (Linux kernel update)
        assert groups[1] == groups[2], f"Windows articles not grouped: {groups}"
        assert groups[3] != groups[1], f"Linux article incorrectly grouped: {groups}"
